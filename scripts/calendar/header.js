...
export const renderHeader = () => {
    ...
    const addDateAndNumber = generateWeekRange(getItem('displayedWeekStart'))
        .map((dayLabel) => `<div             
  class="calendar__day-label day-label">
  <span class="${getCurrentDay(dayLabel, 'day-label__day-name')}">${
                daysOfWeek[dayLabel.getDay()]
            }</span>
  <span class="${getCurrentDay(dayLabel, 'day-label__day-number')}">${   
                dayLabel.getDate()
            }</span>
  </div>`).join('');

    elemCalendarHeader.innerHTML = addDateAndNumber;
};
